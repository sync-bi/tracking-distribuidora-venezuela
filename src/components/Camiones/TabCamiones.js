...
import { Search, Truck, Plus, Users, X, Trash2 } from 'lucide-react';
import TarjetaCamion from './TarjetaCamion';
import { crearCamion, crearConductor, eliminarCamion, eliminarConductor } from '../../services/firestoreService';
          ...
          <div className="grid gap-3">
            {conductores.map(conductor => (
              <div key={conductor.id} className="bg-white p-4 rounded-lg shadow flex items-center justify-between">
                ...
                {conductor.estado === 'Disponible' && (
                  <button onClick={() => handleEliminarConductor(conductor.id)} className="text-red-400 hover:text-red-600">
                    <Trash2 size={18} />
                  </button>
                )}
              </div>
